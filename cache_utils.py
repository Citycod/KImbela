    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            try:
                ...
                if current_app:
                    current_app.logger.debug(f"Cache get failed, will compute fresh: {e}")
    ...
    if hasattr(cache, 'delete'):
        ...
        if not pattern.endswith('*'):
            ...
            if isinstance(cache, SimpleDictCache):
                keys_to_delete = [k for k in cache._cache.keys() if k.startswith(pattern_prefix)]
                for key in keys_to_delete:
                    cache.delete(key)
                if current_app and keys_to_delete:
                    current_app.logger.debug(f"Invalidated {len(keys_to_delete)} keys matching: {pattern}")
