...
@user.route('/view_requests', methods=['GET'])
@login_required
def view_requests():
...
@user.route("/api/requests", methods=["GET"])
@login_required
def get_requests():
    ...
    try:
        ...
        # MODIFIED: Include current user's requests too, but mark them as owned by current user
        ...
        current_app.logger.info(f"Base query would return {query.count()} requests")
        ...
        if location:
            # Join with User table to filter by location
        ...
        current_app.logger.info(f"Found {pagination.total} total requests, showing {len(pagination.items)} on page {page}")
        ...
        for req in pagination.items:
            ...
            # NEW: Check if this request belongs to current user
            ...
            requests_data.append({
                ...
                'is_liked': is_liked,
                'is_own_request': is_own_request  # NEW: Flag to identify own requests
            })
...
@user.route('/api/requests/<int:request_id>')
@login_required
...
def create_matchmaking_request():
    """Create a new matchmaking request with proper error handling"""
    try:
        data = request.get_json()
        ...
        new_request = MatchmakingRequest(
            ...
            payment_gateway=request_data.get('payment_method', 'paystack')
        )
...
@user.route('/api/requests/<int:request_id>/like', methods=['POST'])
@login_required
...
@user.route('/api/requests/<int:request_id>/message', methods=['POST'])
@login_required
def send_message(request_id):
    ...
    try:
        ...
        # Create message (you'll need to integrate with your messaging system)
...
@user.route('/requests/<int:request_id>/deactivate', methods=['POST'])
@login_required
# def deactivate_request(request_id):
#     """Deactivate a matchmaking request"""
#     try:
#         req = MatchmakingRequest.query.filter_by(
#             id=request_id,
#             user_id=current_user.id
#         ).first_or_404()
...
@user.route('/update-payment-status', methods=['POST'])
@login_required
def update_payment_status():
    """Update payment status for a matchmaking request"""
    try:
        data = request.get_json()
        request_id = data.get('request_id')
        status = data.get('status')
        
        matchmaking_request = MatchmakingRequest.query.filter_by(
            id=request_id,
            user_id=current_user.id
        ).first()
        
        if matchmaking_request:
            matchmaking_request.payment_status = status
            if status == 'completed':
...
def payment_cancel():
    ...
    if request_id:
        # Optionally delete the pending request or mark it as cancelled
        matchmaking_request = MatchmakingRequest.query.get(request_id)
        if matchmaking_request and matchmaking_request.user_id == current_user.id:
            matchmaking_request.status = 'cancelled'
            db.session.commit()
...
@user.route('/verify-payment', methods=['POST'])
@login_required
def verify_payment():
...
def payment_success():
    ...
    try:
        ...
        # Redirect to requests page or show success modal
        return redirect(url_for('user.requests'))
...
#         # In production, you would:
#         # 1. Find the request by transaction_id
#         # 2. Update its payment status
#         # 3. If status is 'completed', activate the request
...
@user.route('/api/my-active-request')
@login_required
...
@user.route('/api/requests/<int:request_id>/deactivate', methods=['POST'])
@login_required
def deactivate_request(request_id):
    """Deactivate a matchmaking request"""
    try:
...
@user.route('/api/requests/<int:request_id>/extend', methods=['POST'])
@login_required
