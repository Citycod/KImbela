...
from models import MatchmakingPackage, MatchmakingRequest, MatchmakingLike, MatchmakingView, User, MatchmakingPayments
    ...
    try:
        ...
        for req in pagination.items:
            ...
            requests_data.append({
                ...
                'interests': interests,
                'lifestyles': req.get_lifestyles() if hasattr(req, 'get_lifestyles') else [],
                'image': req.image,
...
def get_request_detail(request_id):
    ...
    try:
        ...
        current_app.logger.error(f"Error fetching matchmaking request {request_id}: {str(e)}")
        return jsonify({
            'success': False,
            ...
            if existing_request.end_date and existing_request.end_date <= datetime.utcnow():
                ...
                return jsonify({
                    ...
                    'existing_request': {
                        ...
                        'days_remaining': (existing_request.end_date - datetime.utcnow()).days if existing_request.end_date else 0,
...
def initiate_matchmaking_payment():
    ...
    try:
        ...
        print(f"🟡 [INITIATE MATCHMAKING PAYMENT] request_id: {request_id}, currency: {currency}, package_id: {package_id}")

        if not request_id or not package_id:
            print("🔴 [INITIATE MATCHMAKING PAYMENT] Request ID and Package ID are required")
            return jsonify({
                'success': False, 
        ...
        if not matchmaking_request:
            print(f"🔴 [INITIATE MATCHMAKING PAYMENT] Matchmaking request not found: {request_id}")
            return jsonify({
                'success': False, 
                'error': 'Matchmaking request not found'
            }), 404

        if matchmaking_request.user_id != current_user.id:
            print(f"🔴 [INITIATE MATCHMAKING PAYMENT] Unauthorized access to request {request_id}")
            return jsonify({
                'success': False, 
        ...
        if matchmaking_request.payment_status == 'completed':
            print(f"🔴 [INITIATE MATCHMAKING PAYMENT] Request already paid: {request_id}")
            return jsonify({
                'success': False, 
        ...
        if str(matchmaking_request.package_id) != str(package_id):
            print(f"🔴 [INITIATE MATCHMAKING PAYMENT] Package mismatch: request has {matchmaking_request.package_id}, got {package_id}")
            return jsonify({
                'success': False, 
        ...
        if not package:
            print(f"🔴 [INITIATE MATCHMAKING PAYMENT] Package not found for request {request_id}")
            return jsonify({
                'success': False, 
        ...
        if package.price is None or package.price <= 0:
            print(f"🔴 [INITIATE MATCHMAKING PAYMENT] Invalid package price for request {request_id}: {package.price}")
            return jsonify({
                'success': False, 
                'error': 'Package price is invalid'
            }), 400

        print(f"🟡 [INITIATE MATCHMAKING PAYMENT] Found package: {package.name}, price: {package.price}")

        # Use dedicated MatchmakingPaymentService
        payment_service = MatchmakingPaymentService()

        print(f"🟡 [INITIATE MATCHMAKING PAYMENT] Calling create_matchmaking_payment...")
        ...
        if result.get('success'):
            print(f"✅ [INITIATE MATCHMAKING PAYMENT] Payment initiated successfully for request {request_id}")
            return jsonify(result)
        else:
            error_msg = result.get('error', 'Payment initiation failed')
            print(f"🔴 [INITIATE MATCHMAKING PAYMENT] Payment initiation failed: {error_msg}")
...
def payment_callback():
    ...
    try:
        ...
        print(f"🟡 [PAYMENT CALLBACK] Received callback - Status: {status}, TX_REF: {tx_ref}, Transaction ID: {transaction_id}")
        ...
        if status == 'successful' and flutterwave_data.get('status') == 'successful':
            ...
            if success:
                ...
                flash('Payment successful! Your matchmaking request is now active.', 'success')
                return redirect(url_for('match.view_requests'))
            else:
                print(f"🔴 [PAYMENT CALLBACK] Failed to process successful payment")
                flash('Error activating your matchmaking request. Please contact support.', 'error')
                return redirect(url_for('match.requests'))
        else:
            # Failed payment
        ...
        import traceback
        current_app.logger.error(f"Payment callback traceback: {traceback.format_exc()}")
...
def like_request(request_id):
    ...
    try:
        ...
        if req.user_id == current_user.id:
            return jsonify({
                'success': False,
                'error': 'You cannot like your own request'
            }), 400
...
def send_message(request_id):
    ...
    try:
        ...
        db.session.rollback()
        current_app.logger.error(f"Error sending message to request {request_id}: {str(e)}")
        return jsonify({
            'success': False,
...
def my_requests():
    ...
    try:
        requests = MatchmakingRequest.query.filter_by(
            user_id=current_user.id
        ).order_by(MatchmakingRequest.created_at.desc()).all()
        ...
        for req in requests:
            requests_data.append({
                ...
                'package': req.package.name,
                'is_active': req.status == 'active' and req.end_date > datetime.utcnow()
            })
    ...
    try:
        ...
        return jsonify({
            ...
            'request': {
                ...
                'package': active_request.package.name if active_request.package else 'Basic',
        ...
        return jsonify({
            'success': False,
            'error': 'Failed to fetch active request'
        }), 500
...
def deactivate_request(request_id):
    ...
    try:
        ...
        return jsonify({
            'success': False,
            'error': 'Failed to deactivate request'
        }), 500
...
def extend_request(request_id):
    ...
    try:
        ...
        if request_to_extend.status != 'active':
            return jsonify({
                'success': False,
                'error': 'Cannot extend inactive request'
            }), 400
        ...
        return jsonify({
            'success': False,
            'error': 'Failed to extend request'
        }), 500
...
def upload_image():
    ...
    try:
        ...
        if file and allowed_file(file.filename, image_extensions):
            ...
            return jsonify({'success': False, 'error': 'Invalid file type. Please upload an image (PNG, JPG, JPEG, GIF, WEBP)'}), 400
...
def get_stats():
    ...
    try:
        ...
        total_matches = db.session.query(db.func.sum(MatchmakingRequest.matches)).scalar() or 0
        new_this_week = MatchmakingRequest.query.filter(
            MatchmakingRequest.created_at >= datetime.utcnow() - timedelta(days=7)
        ).count()
        
        user_requests = MatchmakingRequest.query.filter_by(user_id=current_user.id).count()
...
def debug_requests():
    ...
    for req in requests:
        result.append({
            ...
            'is_active': req.status == 'active' and (req.end_date is None or req.end_date > datetime.utcnow())
        })
    return jsonify(result)
...
def debug_all_requests():
    ...
    for req in all_requests:
        is_active = req.status == 'active' and (req.end_date is None or req.end_date > current_time)
        result['requests'].append({
            'id': req.id,
            'user_id': req.user_id,
            'status': req.status,
            'payment_status': req.payment_status,
            'about_you': req.about_you[:100] + '...' if req.about_you and len(req.about_you) > 100 else req.about_you,
            'end_date': req.end_date.isoformat() if req.end_date else None,
            'is_active': is_active,
            'is_current_user': req.user_id == current_user.id,
            'days_remaining': (req.end_date - current_time).days if req.end_date and req.end_date > current_time else 0,
...
def test_payment_with_keys():
    ...
    try:
        ...
        print(f"✅ [TEST PAYMENT] Step 3: API request completed - Status: {test_response.status_code}")
        
        return jsonify({
            'success': True,
            'keys_configured': True,
            'test_api_status': test_response.status_code,
            'test_api_message': 'API connection successful' if test_response.status_code == 200 else f'API returned {test_response.status_code}',
    ...
    try:
        ...
        return jsonify({
            ...
            'environment_variables': {
                ...
                'FLUTTERWAVE_PUBLIC_KEY': os.getenv('FLUTTERWAVE_PUBLIC_KEY') is not None,
                'FLUTTERWAVE_SECRET_KEY': os.getenv('FLUTTERWAVE_SECRET_KEY') is not None,
            }
        })
    except Exception as e:
        return jsonify({
