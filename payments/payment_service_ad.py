...
class AdCampaignPaymentService:
    ...
    def create_ad_campaign_payment(self, user, campaign, currency='USD'):
        ...
        try:
            ...
            print(f"🟡 [AD PAYMENT] Using Secret Key: {self.FLW_SECRET_KEY[:20]}..." if self.FLW_SECRET_KEY else "🔴 [AD PAYMENT] No secret key!")
            ...
            if response.status_code == 200:
                ...
                print(f"🟡 [AD PAYMENT] Flutterwave response: {json.dumps(result, indent=2)}")
                ...
                print(f"🔴 [AD PAYMENT] HTTP error {response.status_code}: {error_text}")
                return {
                    'success': False,
